...
- Discord Webhook 사용
- 제목, 분류, 게시판에 표시된 날짜/기간, 원문 링크 중심의 기본 메시지로 전송
...
이벤트 게시글은 작성일 대신 이벤트 기간을 `기간:` 라벨로 표시한다.

---

### 4) 중복 알림 방지

- `(board_type, thread_id)` 기준으로 중복 체크
...
- 동일 게시글 중복 전송 없음
- 시스템이 자동으로 반복 실행됨
- 알림은 본문 요약 없이 제목, 분류, 게시판에 표시된 날짜/기간, 링크만 포함함
